...
const vid = { url : `${FanzUrl}`, type : "video/mp4" }
//===========================================================//
...
let photo =["https://telegra.ph/file/ad485f9885a583997db33.jpg"] 
...
newsletterJid:"https://wa.me/c/6285812373011",
serverMessageId: 100,
newsletterName: "FanzBotzz"
}, 
...
thumbnailUrl: lolo,
sourceUrl:"-",
mediaUrl:"https://wa.me/c/6285812373011"
}
}
...
let url = `-`
let anu = "https://telegra.ph/file/509d0541f75f9edc4feec.jpg";
...
let img = { url : `${FanzUrl}`, type : "image/jpeg" }
let url = `-`
...
await conn.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${TextT}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl:`${FanzUrl}`,sourceUrl: `https://wa.me/${nomerOwner}`																												
...
let url = `-`
let links = [
"https://telegra.ph/file/0783edfae88d0c7d41448.jpg",
"https://telegra.ph/file/0783edfae88d0c7d41448.jpg",
"https://telegra.ph/file/0783edfae88d0c7d41448.jpg"
]
...
if(isImage || isQuotedImage){
                   ...
                   conn.sendMessage(num, {caption: teks, image: {url: anu}, mentions: [sender]})
									  await sleep(2000)
									 conn.sendMessage(num, {image: {url: 'https://telegra.ph/file/f7633c2ff07bcc9c71f8c.jpg'}, caption: 'Contoh cara membalas pesan rahasia'})
...
conn.sendMessage(m.chat, {
      ...
      conn.reply(m.sender, `Grub Fanz - Multidevice\n\nhttps://chat.whatsapp.com/-`, m)
}
break
case 'rules':{
let teks =`
...
setReply (mess.wait)
 let { data } = await axios.get(`https://skizo.tech/api/twitterdl?url=${q}&apikey=${api.fanz}`)
...
sendSticker(stikot)
let data = await fetchJson (`https://skizo.tech/api/tiktok?url=${q}&apikey=${api.fanz}`)
...
reply(mess.wait)
let res = await fetchJson(`https://skizo.tech/api/ttsearch?search=${q}&apikey=${api.fanz}`)
...
case  'ig': 
    ...
    sendSticker(stikot)
    let res = await fetchJson (`https://skizo.tech/api/igdl?url=${q}&apikey=${api.fanz}`)
...
if (args.length == 0) return m.reply(`Contoh: ${prefix + command} kids,cute`)
		      m.reply(mess.wait)
 conn.sendMessage(m.chat, { image: { url: `https://skizo.tech/api/txt2img?text=${q}&apikey=${api.fanz}` }, caption: `${botName}\n\n Type: ${command}\n\nText: ${q}`})
 }
			break
 case 'toanime':
case 'jadianime':{
...
let ini_gen = `${command}`
let get_result = await getBuffer(`https://skizo.tech/api/toanime?url=${wewek}&apikey=${api.fanz}`)
conn.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `${botName}`,
...
reply(mess.wait)
conn.sendMessage(from, {image: {url: `https://skizo.tech/api/generatimg?&apikey=${api.fanz}&prompt=${q}`}, caption: 'Nih '}, {quoted: m})
} catch (e){
console.log(e)
reply('apinya kak')
return 
...
if (!isGroup) return onlyGroup()
	...
	setReply(mess.wait)
	conn.sendMedia (from, foto, m, {caption: `FanzBotzz\n*${q}* Nih kak><`})
	})
	} catch(err){
	let teks = args.join(' ')
...
const avatar = await conn.profilePictureUrl( m.quoted ? m.quoted.sender : m.sender,"image").catch(() =>`https://telegra.ph/file/6880771a42bad09dd6087.jpg`)
...
let nono = await jancok.toFile(stok)
let nah = fs.readFileSync(nono)
await conn.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl:'https://telegra.ph/file/5d4d567f3ab24ffd26e1a.jpg',sourceUrl: '-'
}}, sticker: nah }, { quoted: m })   																																
await fs.unlinkSync(stok)
await fs.unlinkSync(media)
} else {
...
if (err) return setReply(err)
let anuah = await TelegraPh(ran)
let mediaa = `https://skizo.tech/api/memegen?url=${anuah}&text=${encodeURIComponent(bottom)}&apikey=${api.fanz}`
...
case 'quotessad':{
  if (!isGroup) return onlyGroup()
let url = `-`
const lan = ['_when lu udah berusaha ngelupain dia yang gabakal mungkin lu miliki, dan sial nya malah ketemu di dalam mimpi_ fnz,','jika tidak bisa bersatu denganmu setidaknya dunia pernah menjadi saksi bahwa aku benar benar mencintaimu_ fnz','_seharusnya dri dlu jngn berharap lebih kpd seseorang_ fnz','_lebih baik tersakiti,daripada harus menyakiti_ fnz','_hidup bukan sebuah perlombaan, nikmati aja setiap proses nya_ fnz','_kita punya harapan tapi semesta punya kenyataan_ fnz','_Ternyata maksain komunikasi sama seseorang yang ga ada perasaan lebih sama kita itu nguras energi banget ya_ fnz,','Dipaksa baik baik saja diatas bumi yang sudah tidak baik baik saja_ fnz','_Terkadang otak kita sudah beradaptasi tanpa kehadiran dia tapi hati sangat sulit untuk kehilangan dia_ fnz','_Malam yg tenang tapi tidak dengan pikiran_','_ngwntod knp dunia ga berpihak sama gw_ fnz','_Kubaca kembali chat lama kita kemudian aku tersenyum dan akhirnya aku menangis_ fnz','_sudah kubilang payung hanya bisa dipakai untuk dua orang. Jika ada yang ketiga, maka salah satunya harus siap d guyur hujan beserta badainya.Tapi ini bukan tentang ☂️ ','Mau balik lagi ?Maaf hubungan ini tidak bisa berlanjut lagi hehe_ fnz','_feling lonley cape?,ya istirahat karna TUHAN tau yang baik gak bikin kamu lonley_','_cape sama dunia nt terus mungkin Tuhan sudah menyiapkan jalan terbaik buat kamu_']
...
conn.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl:`${FanzUrl}`,sourceUrl: `https://wa.me/${nomerOwner}`																										
...
// ========= PRIMBON =========== //
case 'artinama': {
if (!q) return setReply( `Example : ${prefix + command} Fanz Dhoanx`)
...
break
 case 'pasangan': {
if (!q) return setReply( `Example : ${prefix + command} Fanz|Angel`)
...
if (!q) return setReply( `Example : ${prefix + command} Fanz, 28, 6, 2004, angel, 16, 11, 2004`)
...
case 'kecocokannama': 
case 'cocoknama': {
if (!q) return setReply( `Example : ${prefix + command} Fanz, 28, 6, 2004`)
...
break
case 'fengshui': {
if (!q) return setReply( `Example : ${prefix + command} Fanz, 1, 2005\n\nNote : ${prefix + command} Nama, gender, tahun lahir\nGender : 1 untuk laki-laki & 2 untuk perempuan`)
...
break
case 'peruntungan': {
if (!q) return setReply( `Example : ${prefix + command} Fanz, 28, 6, 2004, 2022\n\nNote : ${prefix + command} Nama, tanggal lahir, bulan lahir, tahun lahir, untuk tahun`)
...
conn.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `${week} , ${calender}⌟`,previewType:"PHOTO",thumbnail: its,sourceUrl: `-`,}
...
const mana = bagai[Math.floor(Math.random() * bagai.length)]
 ...
 conn.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `${week} , ${calender}ㅤㅤㅤㅤㅤㅤㅤㅤㅤ`,body:`⌜ ${ucapanWaktu} ⌟`,previewType:"PHOTO",thumbnail: its,sourceUrl: `-`,}
...
conn.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `FanzChan`,body:`⌜ ${ucapanWaktu} ⌟`,previewType:"PHOTO",thumbnail: its,sourceUrl: `-`,}
...
case 'bucinsertifikat':
			...
			case 'bucinserti':
				if (args.length == 0) return setReply(`Example: ${prefix + command} Fanz & angel\n bjir lah love you`)
				setReply(mess.wait)
				let kueri = args.join(" ")
...
case 'blackpink':
		...
		case 'horrorblood':
		case 'thunder':
			if (args.length == 0) return setReply(`Example: ${prefix + command} FanzBotzz`)
            db.data.users[m.sender].limit -= 2 // -1 limit
 setReply(mess.wait)
			conn.sendMessage
			(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${api.Lol}&text=${args}` }, caption: `© FanzBotzz\n\n Type: ${command}\n\nText: ${args}`})
			break
case 'pornhub':
		case 'glitch':
		case 'avenger':
		...
		case 'steel3d':
		case 'wallgravity':
			if (args.length == 0) return setReply(`Example: ${prefix + command} Fanz`)
            db.data.users[m.sender].limit -= 4 // -1 limit
			setReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${api.Lol}&text1=${args[0]}&text2=${args[1]}`}, caption: `© FanzBotzz\n\n Type: ${command}`})
			break
case 'flaming1':{
if (!isGroup) return onlyGroup()
...
conn.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `Fanz°᭄ᴮᵒᵗ`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl:"https://telegra.ph/file/509d0541f75f9edc4feec.jpg",sourceUrl: `https://wa.me/${nomerOwner}`
...
conn.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: 'https://telegra.ph/file/509d0541f75f9edc4feec.jpg',sourceUrl: `https://wa.me/${nomerOwner}`
}}, image: {url:buffer}, caption: `${mess.success}`}, { quoted: m})
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
...
case 'lighteffects':{
if (!isGroup) return onlyGroup()
if (!q) return setReply(`Example : ${prefix+command} Fanz`) 
sendSticker(stikot) 
let link
...
if (args[0].startsWith(`+${6285812373011}`)) return setReply('Tidak bisa call ke nomor ini!')
...
try{
let res = await fetch(`https://skizo.tech/api/tiktok?url=${budy}&apikey=${api.fanz}`)
...
conn.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: `${FanzUrl}`,sourceUrl: `https://wa.me/${nomerOwner}`}},video: { url: json.data.play }, caption: fbk }, { quoted: m })
...
try{
let res = await fetchJson (`https://skizo.tech/api/igdl?url=${budy}&apikey=${api.fanz}`)
for (let i of res.media) {
await sleep (100)
conn.sendMessage(m.chat, { react: { text: "⏳", key: m.key, }})
